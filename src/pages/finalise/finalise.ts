...
export class FinalisePage {
  ...
  slideNext(bundle: any) {
    ...
    switch (bundle) {
      case 'starter':
        ...
        break;
      case 'standard':
        this.abonnementProvider.startCommande(this.concours.id, bundle).then(data => {
          this.standard = data;
  ...
  confirmCommende() {
    switch (this.commande.plan) {
      case 'starter':
        ...
        this.abonnementProvider.confirmFreeCommende(this.commande.id, status).then(data => {
          this.dismiss();
        }, error => {
  ...
  slidePrevious() {
    ...
    if (this.commande)
      this.abonnementProvider.cancelCommande(this.commande.id).then(data => {
      }, error => {
      })
